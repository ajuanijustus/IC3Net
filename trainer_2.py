...
class Trainer(object):
    ...
    def compute_grad(self, batch):
        ...
        if self.args.phase == 1:
            ...
            actions_flat = actions.contiguous().view(-1, dim_actions)
            ...
            value_loss = (values - returns).pow(2).view(-1) * alive_masks
        ...
        if self.args.phase == 2:
            env_action_out = action_out[:-1]
            comm_action_out = [action_out[-1]]
            
            env_actions = actions[:, :, :-1].contiguous().view(-1, dim_actions - 1)
            comm_actions = actions[:, :, -1:].contiguous().view(-1, 1)

            env_log_p_a = [env_action_out[i].view(-1, num_actions[i]) for i in range(dim_actions - 1)]
            comm_log_p_a = [comm_action_out[0].view(-1, 2)]
        else:  # Phases 3 and 4
